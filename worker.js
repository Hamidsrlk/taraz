...
// Cloudflare Workers alternative to api/chat.js (Vercel).
// Deploy with wrangler:
//   npx wrangler deploy worker.js
//   npx wrangler secret put OPENROUTER_API_KEY
// The key is exposed to this script only as the global binding OPENROUTER_API_KEY.
// Route: POST {https://<worker>/api/chat}
...
async function handleRequest(request) {
  ...
  const apiKey = typeof OPENROUTER_API_KEY === 'string' ? OPENROUTER_API_KEY : '';
  if (!apiKey) {
  ...
  for (const model of MODELS) {
    try {
      ...
      if (reply) {
        return json(200, { replyText: reply });
      }
    } catch (e) {
